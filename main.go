...
func main() {
	...
	if err != nil { // MASTER
		...
		go IMA(master, get_array, job_queue, que, last_queue, last_floor, master_order)
		// Println("Starter IMA...")
		master <- true
		...
		go Connect_to_MASTER(get_array, new_master, int_order, ext_order, last_floor, job_queue, que, last_queue)
		new_master <- true
		go Do_first(que)
	}
